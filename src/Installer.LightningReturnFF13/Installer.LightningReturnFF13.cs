...
public partial class LightningReturnFf13 : Form
{
    ...
    private void LightningReturnFf13_Load()
    {
        IList<ResourceInfo> resourcesList = new List<ResourceInfo>(11)
        {
            ...
            new() { Name = "Lightning_Returns_Final_Fantasy_XIII_Logo.webp", FullPath = "images\\Lightning_Returns_Final_Fantasy_XIII_Logo.webp"},
        ...
        {
            ResourceInfo? resObjList = resourcesList.FirstOrDefault(resourceObj => resourceName.EndsWith(resourceObj.Name));
            if (resObjList == null)  continue;

            string dir = Path.GetFullPath(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "wwwroot", resObjList.FullPath));
